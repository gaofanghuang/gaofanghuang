<template>
    <div class="markdown-editor">
        <div class="markdown-tools">
            <div class="markdown-tool" @click="isPreview = false" :class="{'active': !isPreview}">Code</div>
            ...
            <el-popover placement="top" width="320" v-model="showEmoji">
                <div class="markdown-tools-emoji" v-show="showEmoji">
                    <span @click="addEmoji(item)" v-for="(item, key) in emoji" :key="`emoji-${key}`">{{ item }}</span>
                </div>
                <div slot="reference" class="markdown-tool">Emoji</div>
            </el-popover>
        </div>
        <textarea class="markdown-input" v-show="!isPreview" spellcheck="false" autocapitalize="off" autocomplete="off"
            autocorrect="off" v-model="value"></textarea>
