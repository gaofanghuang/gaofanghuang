      ...
      >
        ...
        <template v-if="curItem.type === 1">
          <el-form-item label="需求">
            <div class="log-item-todolist">
              <div
                class="log-item-todoitem"
                v-for="item in curItem.todo"
                :key="`todo-${item.id}`"
              >
        ...
        <template v-if="curItem.type === 3">
          <el-form-item label="图集">
            <div class="log-item-imgs">
              ...
              <div
                class="log-item-img"
                v-for="item in curItem.imgs"
                :key="item.id"
              >
                <img :src="item.path | getImg">
                <i
                  class="log-item-img-delete el-button--danger"
        ...
        <template v-if="curItem.type === 4">
          <el-form-item label="视频">
            <el-input
              v-model="curItem.video"
              placeholder="请输入B站AV号"
            ></el-input>
          </el-form-item>
        </template>
    ...
    >
      ...
      <template v-if="curItem.type === 1">
        <div class="log-view-todolist">
          <div
            class="log-view-todoitem"
            v-for="item in curItem.todo"
            :key="`todo-${item.id}`"
          >
            <span class="log-view-todo-check"><i
                v-if="item.checked"
                class="el-icon-check"
              ></i></span>
      ...
      <template v-if="curItem.type === 2">
        <div
          class="log-view-content markdown-view"
          v-html="viewContent"
        ></div>
      </template>
...
export default {
  ...
  computed: {
    ...
    info() {
      return this.projectList.length > 0
        ? this.projectList.find(item => item.id == this.id)
        : {};
    },
    viewContent() {
  ...
  methods: {
    ...
    handleDelete(row) {
      this.$confirm(`此操作将永久删除记录【${row.id}】, 是否继续?`, "提示", {
        ...
        .then(() => {
          ...
          api.deleteLog(params).then(({ data }) => {
            this.$message({
              type: "success",
    ...
    saveLog() {
      ...
      api.updatedLog(params).then(({ data }) => {
        this.$message({
          message: data.message,
          type: "success"
        });
