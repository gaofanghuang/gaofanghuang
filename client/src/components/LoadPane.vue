<template>
  ...
  >
    ...
    <div
      ref="head"
      v-show="!noSth"
      class="loadpane-head loadpane-head-box"
      :style="{ transform: `translateY(${tY - topBarHeight}px)`, transitionDuration }"
    >
      <slot name="head">
        <div v-if="refreshing" class="loadpane-loading loadpane-icon">
          <span />
          <span />
          <span />
          <span />
    ...
    >
      <slot />
      <span class="search-result-none" v-if="noSth && isSearch">
        <i class="icon icon-no-result"></i>
        <p>没有搜索到相关内容，请修改关键词再试试</p>
      </span>
      <span class="search-result-none" v-if="noSth && !isSearch">
        <i class="icon icon-no-content"></i>
        <p>暂无内容～</p>
      </span>
      <!-- 底部 -->
      <div
        v-if="bottomStatus > 0 && showBottom"
        :class="`loadpane-foot status-${bottomStatus}`"
        @click="beforeLoadMore"
      >
        <slot name="foot">
          <div v-if="bottomStatus === 1" class="loadpane-loading loadpane-icon">
            <span />
            <span />
            <span />
            <span />
          ...
          <span v-if="!noSth">{{ bottomTxt }}</span>
        </slot>
      </div>
    </div>
  </div>
