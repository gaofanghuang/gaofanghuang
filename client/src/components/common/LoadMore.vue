<template>
    <div class="loadmore" style="transform:translate(0,0)" @mousedown="startDrag" @touchstart="startDrag"
        @mousemove.stop="onDrag" @touchmove.stop="onDrag" @mouseup="stopDrag" @touchend="stopDrag" @mouseleave="stopDrag">
        <!-- 顶部 -->
        <div ref="head" class="loadmore-head loadmore-head-box" :style="{transform: `translateY(${ tY - topBarHeight }px)`, transitionDuration}">
            <slot name="head">
                <div v-if="refreshing" class="loadmore-loading loadmore-icon">
                    <span />
                    <span />
                    <span />
                    <span />
                ...
                <i v-else class="loadmore-icon" :style="{ transform: `rotate(${topStatus ? '180deg' : '0'})` }">↓</i>
                <span>{{ topTxt }}</span>
            </slot>
        </div>
        <!-- 内容 -->
        <div class="loadmore-main" :style="{transform: `translateY(${tY}px)`, transitionDuration }">
            <slot />
            <!-- 底部 -->
            <div v-if="bottomStatus > 0 && showBottom" :class="`loadmore-foot status-${bottomStatus}`" @click="beforeLoadMore">
                <slot name="foot">
                    <div v-if="bottomStatus === 1" class="loadmore-loading loadmore-icon">
                        <span />
                        <span />
                        <span />
                        <span />
...
<script>
    ...
    export default {
        ...
        computed: {
            maxDistance() {
                return this.topDistance > 0 ?
                    this.topDistance :
                    this.topBarHeight * 1.5;
            },
            tY() {
